    ...
    public function execute() {
        try {
            if (request::getInstance()->isMethod('POST') and request::getInstance()->isAjaxRequest()) {
                ...
                ambienteHistorialLoteTableClass::delete($ids); //se cambio a true(verdadero)por que por defecto trae false(falso)y ese falso lo elimina total de la tabla y el true lo cambia de estado;
                $this->answer = array(
                    'code' => 200
                );
                session::getInstance()->setSuccess('El registro fue eliminado satisfactoriamente');
                $this->defineView('delete', 'ambienteHistorialLote', session::getInstance()->getFormatOutput());
            } else {
