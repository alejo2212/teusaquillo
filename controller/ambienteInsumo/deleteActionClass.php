  ...
  public function execute() {
    try {
      if (request::getInstance()->isMethod('POST') and request::getInstance()->isAjaxRequest()) {
        ...
        ambienteInsumoTableClass::delete($ids,TRUE);// se cambio a true para borrar logicamente no aparece en la rejilla pero en BD si esta.
        $this->answer = array(
          'code' => 200
        );
        session::getInstance()->setSuccess('El registro fue eliminado satisfactoriamente');
        $this->defineView('delete', 'ambienteInsumo', session::getInstance()->getFormatOutput());
      } else {
