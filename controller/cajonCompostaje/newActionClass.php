...
class newActionClass extends controllerClass implements controllerActionInterface {
  ...
  public function execute() {
      if (session::getInstance()->hasAttribute('form')) {
            $cajonCompostaje = session::getInstance()->getAttribute('form');
            session::getInstance()->setFlash('cajonCompostaje', $cajonCompostaje);
        }
    $this->defineView('new', 'cajonCompostaje', session::getInstance()->getFormatOutput());
  }
