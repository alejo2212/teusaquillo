...
class newActionClass extends controllerClass implements controllerActionInterface {
  ...
  public function execute() {
    if (session::getInstance()->hasAttribute('form')) {
      $clasi = session::getInstance()->getAttribute('form');
      session::getInstance()->setFlash('clasi', $clasi);
    }
//    $this->objclasi = clasificacionPosturaTableClass::getClasificaciones();
    $this->defineView('new', 'clasificacionPostura', session::getInstance()->getFormatOutput());
  }
