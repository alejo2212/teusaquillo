  ...
  public function execute() {
    try {
      if (request::getInstance()->isMethod('POST') and request::getInstance()->isAjaxRequest()) {
        ...
        formaAplicacionTableClass::delete($ids, true);//el true sirve para borrar logicamente
        $this->answer = array(
          'code' => 200
        );
        session::getInstance()->setSuccess('El registro fue eliminado satisfactoriamente');
        $this->defineView('delete', 'formaAplicacion', session::getInstance()->getFormatOutput());
      } else {
