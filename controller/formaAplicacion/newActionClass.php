...
class newActionClass extends controllerClass implements controllerActionInterface {
  ...
  public function execute() {
      if (session::getInstance()->hasAttribute('form')) {
            $formaAplicacion = session::getInstance()->getAttribute('form');
            session::getInstance()->setFlash('formaAplicacion', $formaAplicacion);
        }
    $this->defineView('new', 'formaAplicacion', session::getInstance()->getFormatOutput());
