  ...
  public function execute() {
    try {
      if (request::getInstance()->isMethod('POST')) {
        ...
        foreach ($chk as $data){
            $data = array(
                salidaDetalleIncubadoraTableClass::ID => $data
            );
            salidaDetalleIncubadoraTableClass::delete($data, true);
        }
        
        
        session::getInstance()->setSuccess('Eliminacion Exitosa');
        routing::getInstance()->redirect('salidaIncubadora', 'detail',array('id'=>$id));
        }else{
